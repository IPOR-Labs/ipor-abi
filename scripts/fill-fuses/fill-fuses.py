...
RPC_URLS = {
    "ethereum": "https://eth-mainnet.g.alchemy.com/v2/7l0uPsYLIF0OXzVD78OVTRY5h-PyegII",
    "arbitrum": "https://arb-mainnet.g.alchemy.com/v2/7l0uPsYLIF0OXzVD78OVTRY5h-PyegII",
    "base": "https://base-mainnet.g.alchemy.com/v2/7l0uPsYLIF0OXzVD78OVTRY5h-PyegII"
}
...
def get_contract_deployment_date(web3, address):
    try:
        ...
        # Get the transaction receipt for the contract creation
        # We need to find the block where the contract was created
        # First, let's try to get the transaction count for the address
        # starting from the earliest block
        
        # Binary search approach to find the block where the contract was deployed
        ...
        while left <= right:
            ...
            try:
                ...
                if code == b'' or code == '0x':
                    ...
                    # Contract exists at this block, look in earlier blocks
                    right = mid - 1
            except Exception as e:
                # If error occurs (e.g., block doesn't exist), adjust search
                right = mid - 1
        
        # The deployment block is the first block where the contract has code
...
def update_addresses_json(fuses_file, addresses_file):
    try:
        ...
        # Group fuses by blockchain and name
        blockchain_fuses = {
            "ethereum": {},
            "arbitrum": {},
            "base": {}
        ...
        for file_path, fuses in fuses_data.items():
            ...
            for field_name, address in fuses.items():
                ...
                # Initialize the field_name entry if it doesn't exist
                if field_name not in blockchain_fuses[chain]:
                    blockchain_fuses[chain][field_name] = {}
                
                # Add this version with its deployment date
                blockchain_fuses[chain][field_name][deployment_date] = address

        # Update addresses.json with versioned fuses
        for chain, fuse_dict in blockchain_fuses.items():
            ...
            # Clear existing fuses and add new ones with versions
            fuse_entries = []
            ...
            # Sort fuses by name alphabetically
